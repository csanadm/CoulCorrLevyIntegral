...
im1 = axs[0].imshow(integral_matrix, cmap="Spectral", origin="lower", aspect="auto", extent=[min(df["LogTolerance"])-0.5, max(df["LogTolerance"])+0.5, min(df["NMaxIter"])-0.5, max(df["NMaxIter"])+0.5])
...
im2 = axs[1].imshow(nfunccalls_matrix, cmap="Spectral_r", origin="lower", aspect="auto", extent=[min(df["LogTolerance"])-0.5, max(df["LogTolerance"])+0.5, min(df["NMaxIter"])-0.5, max(df["NMaxIter"])+0.5], norm=LogNorm())
